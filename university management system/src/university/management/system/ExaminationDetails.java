...
public class ExaminationDetails extends JPanel implements ActionListener {
    ...
    public ExaminationDetails(CardLayout cardLayout, JPanel contentPanel) {
        ...
        icon.setBounds(22, 14, 44, 36);
        titleBar.add(icon);

        JLabel title = new JLabel("Examination Details — Check Result");
        title.setFont(FONT_TITLE);
        title.setForeground(Color.WHITE);
        title.setBounds(76, 16, 600, 34);
        titleBar.add(title);

        // Search toolbar
        JPanel tb = new JPanel(new FlowLayout(FlowLayout.LEFT, 12, 10));
        tb.setBackground(Color.WHITE);
        tb.setBounds(0, 65, 1200, 60);
        card.add(tb);

        JLabel sl = new JLabel("Enter / Select Roll No:");
        sl.setFont(FONT_LBL); sl.setForeground(LABEL_COL); tb.add(sl);

        search = new JTextField();
        search.setFont(FONT_FIELD);
        search.setBackground(Color.WHITE);
        search.setForeground(LABEL_COL);
        ...
        search.addFocusListener(new FocusAdapter(){
            public void focusGained(FocusEvent e){search.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(ACCENT,2,true),new EmptyBorder(4,10,4,10)));}
            public void focusLost(FocusEvent e){search.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(BORDER_COL,2,true),new EmptyBorder(4,10,4,10)));}
        });
        tb.add(search);

        result = toolBtn("🔍 Get Result", ACCENT, ACCENT_HOV); result.addActionListener(this); tb.add(result);
        back   = toolBtn("✕ Back", DANGER, new Color(160,40,30)); back.addActionListener(this); tb.add(back);

        // Instruction tip
        JLabel tip = new JLabel("  💡 Tip: Click any row in the table to auto-fill the Roll No above");
        tip.setFont(new Font("Segoe UI", Font.ITALIC, 12));
        tip.setForeground(new Color(100, 120, 150));
        tip.setBounds(0, 125, 800, 22);
        tip.setBackground(new Color(235,243,252));
        tip.setOpaque(true);
        card.add(tip);
    ...
    private JButton toolBtn(String text, Color base, Color hover) {
        ...
        btn.setFont(FONT_BTN);btn.setForeground(Color.WHITE);btn.setPreferredSize(new Dimension(145,36));btn.setBorder(new EmptyBorder(0,0,0,0));btn.setContentAreaFilled(false);btn.setFocusPainted(false);btn.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));return btn;
    }
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == result) {
            ...
            if (!roll.isEmpty()) {
                Marks marksPanel = new Marks(roll, cardLayout, contentPanel);
                contentPanel.add(marksPanel, "marks");
                cardLayout.show(contentPanel, "marks");
            } else {
                JOptionPane.showMessageDialog(this, "Please select or enter a Roll Number.");
            }
