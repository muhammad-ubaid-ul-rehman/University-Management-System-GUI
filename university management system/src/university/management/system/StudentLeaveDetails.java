...
public class StudentLeaveDetails extends JPanel implements ActionListener {
    ...
    JComboBox<String> choiceRollNo;
    JTable table;
    JButton search, cancel, print;
    CardLayout cardLayout;
    JPanel contentPanel;

    public StudentLeaveDetails(CardLayout cardLayout, JPanel contentPanel) {
        ...
        JLabel title = new JLabel("Student Leave Records");
        ...
        JLabel sl = new JLabel("Roll Number:");
        sl.setFont(FONT_LBL); sl.setForeground(LABEL_COL); tb.add(sl);

        choiceRollNo = new JComboBox<>();
        choiceRollNo.setFont(FONT_FIELD); choiceRollNo.setBackground(Color.WHITE);
        choiceRollNo.setPreferredSize(new Dimension(180, 36)); tb.add(choiceRollNo);
    ...
    private void loadAllData() {
        try {
            Conn c = new Conn();
            String q = "SELECT sl.Roll_No, s.Name, sl.Leave_date, sl.Duration " +
                    "FROM studentleave sl JOIN student s ON sl.Roll_No = s.Enrollment_No";
    ...
    private void loadByRoll(String rollNo) {
        try {
            Conn c = new Conn();
            String q = "SELECT sl.Roll_No, s.Name, sl.Leave_date, sl.Duration " +
                    "FROM studentleave sl JOIN student s ON sl.Roll_No = s.Enrollment_No " +
                    "WHERE sl.Roll_No = '" + rollNo + "'";
            ResultSet rs = c.statement.executeQuery(q);
            table.setModel(DbUtils.resultSetToTableModel(rs));
            styleTable(table);
        } catch (Exception ex) { ex.printStackTrace(); }
    ...
    public void actionPerformed(ActionEvent e) {
        if(e.getSource()==search) loadByRoll((String)choiceRollNo.getSelectedItem());
